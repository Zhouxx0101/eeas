...
<script>
import { listNews, getNews, delNews, addNews, updateNews } from "@/api/data/news";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除新闻数据编号为"' + ids + '"的数据项？').then(function() {
        return delNews(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('data/news/export', {
        ...this.queryParams
